...
2. **src/assets/** - Ассеты (изображения, иконки)
...
7. **OSISettings** - Настройки системы
8. **OSITetris** - Игра в тетрис

> **OSICalendar** и **OSIHelper** временно отключены в `applications.js` (закомментированы).
...
- `iconclass` — CSS-классы Bootstrap Icons для иконки
...
2. Зарегистрируйте приложение в `src/config/applications.js` — добавьте экземпляр `OSIApp` **с полем `loader`**:
   ```javascript
   new OSIApp({
       id: 'osinewapp',
       name: 'OSINewApp',
       ...
       iconclass: 'bi-stars text-ico-purpure',
       defWidth: 800,
       defHeight: 450,
       resizable: true,
       canMinimize: true,
...
- Персистентность: IndexedDB (`src/idb/db.js`)
